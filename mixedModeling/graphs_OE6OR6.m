...
%% 
columns = {'sub','ses','timeWin','trialID','block_type','seniority','blockpos','opos','ampAvg_neg'};

variables_name = {'tone_pos_in_trial','sleep_stage','block_pos_in_file','block_type'};
variables = {num2cell(6:10),referenced_filename_ses_types,num2cell(1:24),{'fixed','random'}};
sess = {'wake_morning'}; %{'wake_morning','wake_night'};
...
%     subs_avgErp_var_pos = zeros([numel(subs),138,numel(curr_variable)]);
    ...
    curr_clust_name = 'neg';
    curr_output_filename = sprintf('%s//var_%s_%s_OR6OE6Clust.png',output_dir, curr_variable_name,curr_clust_name);
    ...
    axis([time(1) time(end) -1 1]) %axis([time(1) time(end) -1 1.5])
    xlabel('Time (s)')
    ylabel('µV')
