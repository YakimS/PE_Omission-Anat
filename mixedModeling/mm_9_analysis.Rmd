...
o_sepRF <- read.csv("C:\\Users\\User\\Cloud-Drive\\BigFiles\\OmissionExpOutput\\mix_modeling\\mm_9_sub-ses-lessTimeWin_traits_posnegClust.csv")
...
fixed_vars= c( "blockpos", "ses", "opos","block_type","seniority") #"ses*block_type" will raise an error
...
fixed_vars= c( "blockpos", "timeWin", "ses", "opos", "block_type","seniority") #"ses*block_type" will raise an error
filename = paste("mm_", curr_ver,"_res_sepRF_allFixed+1sub+trailID_within-X_interOnly_timeAsFactor.csv", sep = "")
...
exp_mod_pos_1 <- "ampAvg_pos ~ blockpos + (1|sub)+ (1|trialID)"
exp_mod_neg_1 <- "ampAvg_neg ~ blockpos + (1|sub)+ (1|trialID)"
str_1 = "mm_X_res_sepRF_blockpos+1sub_within-SesTimewinBt.csv"
exp_mod_pos_2 <- "ampAvg_pos ~ opos + (1|sub)+ (1|trialID)"
exp_mod_neg_2 <- "ampAvg_neg ~ opos + (1|sub)+ (1|trialID)"
str_2 = "mm_X_res_sepRF_opos+1sub_within-SesTimewinBt.csv"
exp_mod_pos_3 <- "ampAvg_pos ~ seniority + (1|sub)+ (1|trialID)"
exp_mod_neg_3 <- "ampAvg_neg ~ seniority + (1|sub)+ (1|trialID)"
str_3 = "mm_X_res_sepRF_seniority+1sub_within-SesTimewinBt.csv"
exp_mod_pos_4 <- "ampAvg_pos ~ blockpos + opos + seniority + (1|sub)+ (1|trialID)"
exp_mod_neg_4 <- "ampAvg_neg ~ blockpos + opos + seniority + (1|sub)+ (1|trialID)"
  ...
  for (i in ses_vars) {
    for (j in timeWin_vars) {
      for (k in block_type_vars) {
        # define models
        o_sepRF_pos.null <- lmer(ampAvg_pos ~ (1|sub)+ (1|trialID) , data=subset(o_sepRF, timeWin == j & ses==i & block_type==k), REML=FALSE)
        o_sepRF_neg.null <- lmer(ampAvg_neg ~ (1|sub)+ (1|trialID) , data=subset(o_sepRF, timeWin == j & ses==i & block_type==k), REML=FALSE)
