    ...
    for ses_i=1:numel(referenced_filename_ses_types)
        ...
        for o_i=1:size(o_events_indexes,2)
            for j=1:numel(timewin)
                ...
                mm_mat(mm_i).('opos') = elaborated_events(o_i).tone_pos_in_trial;
                mm_mat(mm_i).('blockpos') = elaborated_events(o_i).block_pos_in_file;
                mm_mat(mm_i).('seniority') = elaborated_events(o_i).omission_type_seniority;
                mm_mat(mm_i).('block_type') = elaborated_events(o_i).block_type;
                mm_mat(mm_i).('trialID') = sprintf('%s_%s_%d',curr_sub,curr_ses,elaborated_events(o_i).event_id);
