function run_spatioTemporalAnalysis(args_path)
    ...
    f.spatiotempoClustPerm_cond1VsCond2_subAvg(cond1_Vs_cond2_dir, contrasts,imp,subs,neig)
    
    %% Cond1 VS Cond2 - per subject
    plot_topoplot = false;
    f.spatiotempoClustPerm_cond1VsCond2_perSub(cond1_Vs_cond2_dir, contrasts,imp,subs,neig,plot_topoplot)
    
    %% baseline vs activity - subjects mean
    plot_topoplot = false;
    f.spatiotempoClustPerm_baselineVsActivity_subAvg(pre_vs_poststim_dir, pre_vs_post_conds_names,imp,subs, bl,neig,plot_topoplot)

    %% Cond1 VS Cond2 - per subject
    plot_topoplot = false;
    f.spatiotempoClustPerm_baselineVsActivity_perSub(pre_vs_poststim_dir, pre_vs_post_conds_names,imp,subs, bl,neig,plot_topoplot)
    
    %% load pre-post-stimuli cluster stats
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    conds_preVsPoststim = {};
    for cond_i= 1:size(pre_vs_post_conds_names,2)
        conds_preVsPoststim.(pre_vs_post_conds_names{cond_i}) = load(sprintf("%s\\preVsPoststim_bl-%d_%s_avg",pre_vs_poststim_dir, bl, pre_vs_post_conds_names{cond_i}));
    end
    %% baseline_avgAndStd_allElectd
    f.baseline_avgAndStd_allElectd(pre_vs_post_conds_names,imp,subs,electrodes)
    %% Is the baseline activity signigicantly different than 0
    f.plot_and_test_basline_different_that_0(pre_vs_post_conds_names,imp,subs,electrodes,dir_baseline_erp,time,neig)
    ...
    f.print_clusters_probability(contrasts,subs,cond1_Vs_cond2_dir)
    
    %% Common time-electrodes matrix for every subjects cond1 vs. cond2 cluster test
    
    f.timeElectedMatrix_perCond1VsCond2Cluster(contrasts,electrodes,cond1_Vs_cond2_dir,subs,time)
    ...
    for cont_i=1:size(contrasts,2)
        ...
        f.plot_OClusters_contrasts(conds,conds_preVsPoststim,subs,time,imp,bl,pre_vs_poststim_dir)
    end
    
    %% Time permutation analysis for each electrode X subj
    f.tempoClustPerm_cond1VsCond2_perSub(contrasts, subs,imp,persub_output_dir,neig,electrodes)
    
    %% Create a graph sums all sig. electrode X subj
    f.plot_allSubs_electrodes_sig_range(contrasts, subs, electrodes,time,persub_output_dir)
    f.plot_allSubs_electrodes_sig_range_omissionAvgElect(contrasts, subs, electrodes,time,bl,persub_output_dir,pre_vs_poststim_dir)
    f.plot_allSubs_electrodes_sig_range_omissionPerSubElect(contrasts, subs, electrodes,time,bl,persub_output_dir,pre_vs_poststim_dir)
end
