...
sleep_stages = {'N2','N3','REM'};
wake_stages = {'wake_night','wake_morning'};

%% wake
for slws_i =  1:numel(wake_stages)
    for var_i = 1:numel(variables_name)
        ...
        for sub_i=1:numel(subs)
            ...
            if strcmp(subs{sub_i},'23') && strcmp(wake_stages{slws_i},'wake_morning')
                continue;
            end
            ...
            for val_i=1:numel(curr_variable)
                ...
                elseif  strcmp(wake_stages{slws_i}, "REM")
                    indexes_value = find( strcmp({elaborated_events.('TOA')},'O')  == 1 ...
                    & strcmp({elaborated_events.(curr_variable_name)},curr_variable{val_i})...
                    & (strcmp({elaborated_events.("sleep_stage")},"Rt") | strcmp({elaborated_events.("sleep_stage")},"Rp")));
                else
                    indexes_value = find( strcmp({elaborated_events.('TOA')},'O')  == 1 ...
                    & strcmp({elaborated_events.(curr_variable_name)},curr_variable{val_i})...
                    & (strcmp({elaborated_events.("sleep_stage")},wake_stages{slws_i})));
                end
       ...
       %%%%%%%   plot
        ...
        for k=1:numel(clusts_res)
            ...
            curr_output_filename = sprintf('%s//var_%s_%s_oe6or6_stage-%s.png',output_dir, curr_variable_name,curr_clust_name,wake_stages{slws_i});
            figure();
            ...
            title_add = "";
            title(sprintf("%d Subject Avg ERP, %s cluster =OR6vsOF6 \n variable: %s%s, sleepstage:%s",numel(subs),curr_clust_name,curr_variable_name,title_add,wake_stages{slws_i}));
            if numel(curr_variable)<=5
...
%% sleep
for slws_i =  1:numel(sleep_stages)
    for var_i = 1:numel(variables_name)
        curr_variable_name = variables_name{var_i};
        curr_variable = variables{var_i};
        ...
        for sub_i=1:numel(subs)
            ...
            for val_i=1:numel(curr_variable)
                ...
                elseif  strcmp(sleep_stages{slws_i}, "REM")
                    indexes_value = find( strcmp({elaborated_events.('TOA')},'O')  == 1 ...
                    & strcmp({elaborated_events.(curr_variable_name)},curr_variable{val_i})...
                    & (strcmp({elaborated_events.("sleep_stage")},"Rt") | strcmp({elaborated_events.("sleep_stage")},"Rp")));
                else
                    indexes_value = find( strcmp({elaborated_events.('TOA')},'O')  == 1 ...
                    & strcmp({elaborated_events.(curr_variable_name)},curr_variable{val_i})...
                    & (strcmp({elaborated_events.("sleep_stage")},sleep_stages{slws_i})));
                end
       ...
       %%%%%%%   plot
        ...
        for k=1:numel(clusts_res)
            ...
            curr_output_filename = sprintf('%s//var_%s_%s_oe6or6_stage-%s.png',output_dir, curr_variable_name,curr_clust_name,sleep_stages{slws_i});
            figure();
            one_div_sqrt_samp_size = 1/sqrt(size(subs,2));
            for val_i=1:numel(curr_variable)
                curr_var_val_name = curr_variable{val_i};
            ...
            title_add = "";
            title(sprintf("%d Subject Avg ERP, %s cluster =OR6vsOF6 \n variable: %s%s, sleepstage:%s",numel(subs),curr_clust_name,curr_variable_name,title_add,sleep_stages{slws_i}));
            if numel(curr_variable)<=5
                legend("Location","northwest","FontSize",10);
            end
