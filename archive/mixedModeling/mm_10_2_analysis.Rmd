...
o_sepRF <- read.csv("C:\\Users\\User\\Cloud-Drive\\BigFiles\\OmissionExpOutput\\mix_modeling\\mm_10_2_sub-ses-76msTimeWin_traits_posnegClust.csv")
...
# Combinations of fixed variables, vs. null 
# Within timeWin
filename = paste("mm_", curr_ver,"_res_sepRF_allFixed+1sub_within-Timewin_interOnly_timeAsFactor.csv", sep = "")
...
exp_mod_pos_1 <- "ampAvg_pos ~ seniority*block_type*ses +(1|sub)"
exp_mod_neg_1 <- "ampAvg_neg ~ seniority*block_type*ses +(1|sub)"
str_1 = "mm_X_res_sepRF_XXXXX+1sub_within-TimewinBt.csv"
...
exp_pos_mods <- list(exp_mod_pos_1)
exp_neg_mods <- list(exp_mod_neg_1)
contrast_str <- list(str_1)
...
for (mod in 1:length(exp_pos_mods)) {
  ...
  for (j in timeWin_vars) {
      ...
      o_sepRF_neg.model<-lmer(as.formula(exp_neg_mods[[mod]]) , data=subset(o_sepRF, timeWin == j), REML=FALSE)
      o_sepRF_pos.model<-lmer(as.formula(exp_pos_mods[[mod]]) , data=subset(o_sepRF, timeWin == j), REML=FALSE)
...
fixed_vars= c( "blockpos", "timeWin", "ses", "opos", "block_type","seniority") 
filename = paste("mm_", curr_ver,"_res_sepRF_oneFixed+1sub_within-X_interAndSlope.csv", sep = "")
