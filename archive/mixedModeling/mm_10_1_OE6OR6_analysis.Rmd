...
curr_ver = "10_1_OE6OR6"

o_sepRF <- read.csv("C:\\Users\\User\\Cloud-Drive\\BigFiles\\OmissionExpOutput\\mix_modeling\\mm_10_1_sub-ses-100msTimeWin_traits_negOR6OE6Clust.csv")
...
filename = paste("mm_", curr_ver,"_res_sepRF_allFixed+1sub_within-TimewinSes_interOnly.csv", sep = "")
...
fixed_vars= c( "blockpos", "timeWin", "opos", "block_type","seniority")
filename = paste("mm_", curr_ver,"_res_sepRF_allFixed+1sub_within-sesnight_interOnly.csv", sep = "")
...
for (mod in all_models_fix_var) {
  for (posneg in posneg_vector) {
    exp_mod <- paste("ampAvg_", posneg ," ~ 1 + ",mod," + (1|sub)+ (1|trialID)", sep = "")
    null_mod <- paste("ampAvg_",posneg ," ~ 1 + (1|sub)+ (1|trialID)", sep = "")
    o_sepRF.model<-lmer(as.formula(exp_mod) , data=subset(o_sepRF, ses=="wake_night"), REML=FALSE)
    o_sepRF.null <- lmer(as.formula(null_mod) , data=subset(o_sepRF, ses=="wake_night"), REML=FALSE)
...
exp_mod_neg_1 <- "ampAvg_neg ~ timeWin+opos+block_type + (1|sub)"
str_1 = "mm_X_res_sepRF_timeWin+opos+block_type+1sub_within-X.csv"
exp_mod_neg_2 <- "ampAvg_neg ~ blockpos+timeWin+opos+block_type + (1|sub)"
str_2 = "mm_X_res_sepRF_blockpos+timeWin+opos+block_type+1sub_within-X.csv"
exp_mod_neg_3 <- "ampAvg_neg ~ timeWin+opos+block_type+seniority + (1|sub)"
str_3 = "mm_X_res_sepRF_timeWin+opos+block_type+seniority+1sub_within-X.csv"
exp_mod_neg_4 <- "ampAvg_neg ~ blockpos+timeWin+opos+block_type+seniority + (1|sub)"
str_4 = "mm_X_res_sepRF_blockpos+timeWin+opos+block_type+seniority+1sub_within-X.csv"

exp_neg_mods <- list(exp_mod_neg_1,exp_mod_neg_2,exp_mod_neg_3,exp_mod_neg_4)
contrast_str <- list(str_1,str_2,str_3,str_4)

for (mod in 1:length(exp_neg_mods)) {
  output_sepRF <- data.frame(fixed_vars = character(), posNeg = character(), Bayes = numeric(), anovaPval = numeric())
  # define models
  o_sepRF_neg.null <- lmer(ampAvg_neg ~ (1|sub) , data=o_sepRF, REML=FALSE)
  o_sepRF_neg.model<-lmer(as.formula(exp_neg_mods[[mod]]) , data=o_sepRF, REML=FALSE)
  ...
  anova_preTerm_out = anova(o_sepRF_neg.model)
  
  # Append variables to the data frame
  newEntry_neg <- data.frame(fixed_vars = exp_neg_mods[[mod]],posNeg = "Neg", Bayes = exp(bayes_out_neg$log_BF[2]), anovaPval = anova_out_neg$"Pr(>Chisq)"[2])
...
Modeling the slope and the intercept

```{r}

# Combinations of fixed variables, vs. null 
# Within timeWin
filename = paste("mm_", curr_ver,"_res_sepRF_oneFixed+1sub_within-Timewin_interAndSlope.csv", sep = "")
...
for (mod in fixed_vars) {
  for (posneg in posneg_vector) {
    for (j in timeWin_vars) {
      exp_mod <- paste("ampAvg_", posneg ," ~ 1 + ",mod," + (",mod,"|sub)", sep = "")
...
filename = paste("mm_", curr_ver,"_res_sepRF_oneFixed+1sub+trailID_within-X_interAndSlope.csv", sep = "")
...
for (mod in fixed_vars) {
  for (posneg in posneg_vector) {
    exp_mod <- paste("ampAvg_", posneg ," ~ 1 + ",mod," + (",mod,"|sub)+ (1|trialID)", sep = "")
    null_mod <- paste("ampAvg_",posneg ," ~ 1 + (1|sub)+ (1|trialID)", sep = "")
