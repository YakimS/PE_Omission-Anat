function results_table = generate_stcp_cluster_table(file_pattern)

% INPUTS:
%   file_pattern  - File pattern to match (e.g., 'path/to/data/STCP-ERP_name-*T1234_clust-centElec.mat')
%
% OUTPUTS:
%   results_table - Table containing cluster statistics
%
% Example usage:
%   results = generate_stcp_cluster_table('path/to/data/STCP-ERP_name-*T1234_clust-centElec.mat');
    ...
    for file_i = 1:numel(files)
        ...
        if ~isfield(data, 'metadata') || ~isfield(data.metadata, 'stat') || ~isfield(data.metadata, 'stat_cfg')
        ...
        n = numel(unique(data.metadata.stat_cfg.design(2,:)));
