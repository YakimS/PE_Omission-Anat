function [flat_table, nested_struct, metadata] = create_component_amplitude_table(subs, sovs, comps, conds, cluster, dirs, epoch_time)
...
%   [flat_table, nested_struct, metadata] = create_component_amplitude_table(subs, sovs, comps, conds, cluster, dirs, time);
    ...
    % Initialize outputs
    flat_table = table('Size', [0 12], ...
                      'VariableTypes', {'string', 'string', 'string', 'string', 'double', 'double', 'logical', 'double', 'string', 'string', 'string', 'string'}, ...
                      'VariableNames', {'subject', 'component', 'sov', 'cond', 'amplitude', 'latency', 'is_peak', 'trials_count', 'color_id','name_cond_long','name_sov_long', 'error_msg'});
...
function result = process_single_combination(sub_name, sov, cond, comp, cluster, dirs, epoch_time)
    ...
    try
        % Get timelocked data
        curr_cond_timelocked = get_cond_timelocked({sub_name}, cond,  sov, dirs.ft_cond_input, dirs.ft_cond_output);
        ...
        comp_time_idx = find(epoch_time >= comp.latency(1) & epoch_time <= comp.latency(2));
