...
%                               
%                                 factor "ses"
%                                    wake_night             N2       N3     REM
% 
% factor          expomit               1                    2        3      4                
% "trialtype"     unexpomit             5                    6        7      8         
...
randpermutations = 30;
sovs = {'wnight','N2','REM','N3'};
output_dir = sprintf("%s\\RESULTS_resampNo_subRandPerm",main_output_dir);
cfg = [];                                  
...
cfg.randompermutations = randpermutations;%200;            % USE ONLY if you want to have within subject decoding significant line
...
for sub_i=1:numel(subs)
    ...
    for cont_i=1:numel(contrasts_structs)
        ...
        randperm_files = dir(sprintf("%s\\ALL_NOSELECTION\\randperm",output_path));
        pattern = sprintf('CLASS_PERF_s-%d.*PERM(\\d+)', sub_i);
        perm_numbers = [];
        ...
        for i = 1:length(randperm_files)
            ...
            if ~isempty(tokens)
                ...
                perm_numbers = [perm_numbers, curr_file];
            end
        end
        if ~isempty(perm_numbers)
            max_perm = max(perm_numbers);
            cfg.randompermutations = randpermutations - max_perm;
            if cfg.randompermutations > 0
                fprintf("Running: sub %d, in sov %s only %d exists\n",sub_i,subset_name,max_perm);
                ...
                fprintf("Done: sub %d, in sov %s all %d exists\n",sub_i,subset_name,randpermutations);
            end
        end
    end
end
...
cfg.channelpool = 'ALL_NOSELECTION';       
cfg.resample = "no";%100;                         % downsample, "no" or sampling rate units(<250hz) (useful for temporal generalization). For test 30
cfg.save_confidence = 'yes';
contrasts_structs = struct('name', {}, 'cond1', {}, 'cond2', {});
...
tic
ticBytes(gcp);
parfor (cont_i=1:numel(contrasts_structs) ,4)
tic
% for cont_i=1:numel(contrasts_structs)
    for sub_i=1:numel(subs)
        sub_input_filename = sprintf("s-%d_allSovs.mat",sub_i);
