classdef ft_importer < handle
    ...
    methods(Static)
        ...
        function subs_cond_timlocked = get_cond_timelocked(o,subs,cond,sov)
            ...
            for sub_i=1:size(subs,2)
                ...
                try
                    ...
                    o.error_if_less_than_5_trials(subs{sub_i},cond, sov, subs_cond_timlocked{sub_i}.cfg.('trials_timelocked_avg'))
                catch ME
                    cfg = [];
                    cfg.feedback = 'no';
        ...
        function error_if_less_than_5_trials(sub, cond, sov, numOfTrials)
                if numOfTrials < 5
                        fprintf('sub: %s, cond: %s, sov: %s', sub, cond.long_s, sov.long_s)
...
%         end
        ...
        function allsubs_cond_tfr=get_cond_TFR(o,subs,cond,sov,tfr_algo)
            if strcmp(tfr_algo,'multitaper')
                allsubs_cond_tfr = o.get_cond_TFR_mt(o,subs,cond,sov);
            elseif strcmp(tfr_algo,'multitaper_zscored')
                allsubs_cond_tfr = o.get_cond_TFR_mt_zscored(o,subs,cond,sov);
            elseif strcmp(tfr_algo,'hilbert')
                allsubs_cond_tfr = o.get_cond_TFR_hilbert(o,subs,cond,sov);
            elseif strcmp(tfr_algo,'hilbert_zscored')
                allsubs_cond_tfr = o.get_cond_TFR_hilbert_zscored(o,subs,cond,sov);
            else
                error('no such tfr algo implemented')
            end
        end
   
        function allsubs_cond_tfrHilbert=get_cond_TFR_hilbert(o,subs,cond,sov)
                allsubs_cond_tfrHilbert = cell(1, size(subs,2));
                for sub_i=1:size(subs,2)
                    filename = sprintf("tfrHilbert_sov-%s_cond-%s_sub-%s.mat",sov.short_s,cond.short_s,subs{sub_i});
                    file_path = sprintf("%s\\tfr_hilbert\\%s",o.output_dir,filename);
                    try
                        loaded = load(file_path);
                        allsubs_cond_tfrHilbert{sub_i} = loaded.tfrHilbert_subcond;
                        o.error_if_less_than_5_trials(subs{sub_i},cond, sov, allsubs_cond_tfrHilbert{sub_i}.cfg.('trials_avg'))
                    catch ME
                        fprintf("Running: %s\n",filename);
                        cfg = [];
                        cfg.feedback = 'no';
...
%%%%                    Hilbert - ft func
                        ...
                        save(file_path,"tfrHilbert_subcond", '-v7.3')
                    end
                end
        end

        function allsubs_cond_tfrHilbert_zscored=get_cond_TFR_hilbert_zscored(o,subs,cond,sov)
                ...
                for sub_i=1:size(subs,2)
                    ...
                    file_path = sprintf("%s\\tfr_hilbert\\zscored\\%s",o.output_dir,filename);
                    try
                        loaded = load(file_path);
                        allsubs_cond_tfrHilbert_zscored{sub_i} = loaded.tfrHilbertzscored_subcond;
                        o.error_if_less_than_5_trials(subs{sub_i},cond, sov, allsubs_cond_tfrHilbert_zscored{sub_i}.cfg.('trials_avg'))
        ...
        function allsubs_cond_tfrMT_zscored=get_cond_TFR_mt_zscored(o,subs,cond,sov)
            allsubs_cond_tfrMT_zscored = cell(1, size(subs,2));
            for sub_i=1:size(subs,2)
                filename = sprintf("tfrMTzscored_sov-%s_cond-%s_sub-%s.mat",sov.short_s,cond.short_s,subs{sub_i});
                file_path = sprintf("%s\\tfr_mt\\zscored\\%s",o.output_dir,filename);
                try
                    loaded = load(file_path);
                    allsubs_cond_tfrMT_zscored{sub_i} = loaded.tfrMTzscored_subcond;
                    o.error_if_less_than_5_trials(subs{sub_i},cond, sov, allsubs_cond_tfrMT_zscored{sub_i}.cfg.('trials_avg'))
                catch ME
                    fprintf("Running: %s\n",filename);
                    cfg = [];
                    cfg.feedback = 'no';
        ...
        function allsubs_cond_tfrMT=get_cond_TFR_mt(o,subs,cond,sov)
            allsubs_cond_tfrMT = cell(1, size(subs,2));
            for sub_i=1:size(subs,2)
                filename = sprintf("tfrMT_sov-%s_cond-%s_sub-%s.mat",sov.short_s,cond.short_s,subs{sub_i});
                file_path = sprintf("%s\\tfr_mt\\%s",o.output_dir,filename);
                try
                    loaded = load(file_path);
                    allsubs_cond_tfrMT{sub_i} = loaded.tfrMT_subcond;
                    o.error_if_less_than_5_trials(subs{sub_i},cond, sov, allsubs_cond_tfrMT{sub_i}.cfg.('trials_avg'))
...
%%%                    Multitaper - ft func
                    ...
                    save(file_path,"tfrMT_subcond", '-v7.3')
                end
            end
        end
            ...
            if ~isfield(o.tfrHilbert,sprintf("%s_%s",sov.short_s,cond.short_s))
                ...
                for sub_i=1:size(o.subs,2)
                    ...
                    try
                        ...
                        error_if_less_than_5_trials(o.subs{sub_i},cond.long_s, sov.long_s,allsubs_cond_tfrWavelets{sub_i}.cfg.('trials_avg'))
                    catch ME
                        fprintf("Running: %s\n",filename);
                        cfg = [];
                        cfg.feedback = 'no';
