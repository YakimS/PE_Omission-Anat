...
clear
%%
addpath('ATO_configuration.m')
[v, subs,dirs,time, events] = ATO_configuration();
...
% f = ADAPTATION_get_funcs_instant(subs,{v.wn}, {v.AOmi},dirs.ft_cond_input,dirs.ft_cond_output,time);
% arbitrary_cond = f.imp.get_cond_timelocked(f.imp,f.imp.subs,v.AOmi,v.wn);
...
for sov_i=1:numel(sovs)
    for cont_i=1:numel(contrasts)
        f = AOT_get_funcs_instant(subs,{sovs{sov_i}}, contrasts{cont_i},dirs.ft_cond_input,dirs.ft_cond_output,time);
        f.run_STCP_ERP_dependent(f,comp_output_dir,contrasts{cont_i},{sovs{sov_i},sovs{sov_i}}, cfg)
    end
end
