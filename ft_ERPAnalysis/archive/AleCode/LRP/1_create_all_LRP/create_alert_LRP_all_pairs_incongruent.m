...
function create_alert_LRP_all_pairs_incongruent(basename)
...
%%
eeglab nogui
% EEG = pop_loadset('filename', [EEG.filename], 'filepath',[filepath_alert_neutro]);
EEG = pop_loadset('filename', [EEG.filename], 'filepath',[filepath_alert_sinAMA ]);

%% COUNT TOTAL  EPOCHS
EEG = pop_selectevent( EEG, 'type',[71 74],'deleteevents','off','deleteepochs','on','invertepochs','off');
...
%% RIGHT RESPONSE 74
EEG = pop_selectevent( EEG, 'type',74,'deleteevents','off','deleteepochs','on','invertepochs','off');
EEG = eeg_checkset( EEG ); 
...
cfg         = [];
cfg.latency = [-0.2 0.996];%cfg.removemean         = 'no'
...
EEG = pop_selectevent( EEG, 'type',71,'deleteevents','off','deleteepochs','on','invertepochs','off');
EEG = eeg_checkset( EEG ); 
if length(EEG.epoch)<21
   fprintf('%s%s%s%s\n', 'There is less than 21 trials. Epochs =  ',num2str(length(EEG.epoch)),' Subject:',[EEG.filename])
   return
...
save([basename(1:15) '_lrp_alert_incongruent.mat'], 'lrp');
end

%% load single subject ERPs in a single cell array
% all_lrp= cell(1,length(list_participants));
% for i=1:length(list_participants)
%     load([list_participants{i}(1:15) '_lrp_alert_incongruent.mat']);
%     all_lrp{i} = lrp;
% end
% save(['all_lrp_alert_incongruent.mat'], 'all_lrp');
% 

% % %% compute grand average across subjects
% % %% compute grand average across subjects
% % clear;clc;
% % path_data = 'C:\Toolboxes\conflict_decoding\alert';
% % load('all_lrp_alert_congruent.mat')
% % % cfg         = [];
% % % cfg.latency = [-0.2 1.996];
% % % avg_LRP_alert_cong    = ft_timelockgrandaverage(cfg,all_lrp{:});
% % % save(fullfile(path_data, 'avg_LRP_alert_congruent.mat'), 'avg_LRP_alert_cong');
% % % 
% % % %%
% % % cfg             = [];
% % % clf;
% % % ft_singleplotER(cfg, all_lrp{:})