...
%% create "noN2EventsAblO","noN2EventsAblOF","noN2EventsAblOR"

conds = {"noN2EventsAblO","noN2EventsAblOF","noN2EventsAblOR"};
...
for sub_i = 1:numel(subs)
    ...
    for files_i = 1:length(files_set)
        ...
        for events_type_i=1:numel(conds)
            if strcmp(conds{events_type_i},'noN2EventsAblO')
                curr_events = find(strcmp({sub_events.TOA}, 'O') & strcmp({sub_events.sleep_stage},'N2'));
            elseif strcmp(conds{events_type_i},'noN2EventsAblOF')
                curr_events = find(strcmp({sub_events.TOA}, 'O') & strcmp({sub_events.block_type}, 'fixed') & strcmp({sub_events.sleep_stage},'N2'));
            elseif strcmp(conds{events_type_i},'noN2EventsAblOR')
                curr_events = find(strcmp({sub_events.TOA}, 'O') & strcmp({sub_events.block_type}, 'random') & strcmp({sub_events.sleep_stage},'N2'));
            else
            ...
            for i = 1:length(curr_events)
                currEventID = sub_events(curr_events(i)).event_id;
                for j = 1:length(n2_sleepevents_reref_table)
                    if n2_sleepevents_reref_table(j).event_id == currEventID ...
    ...
    for files_i = 1:length(files_set)
        ...
        for events_type_i=1:numel(conds)
            ...
            elseif strcmp(conds{events_type_i},'AOR')
                curr_events = find(strcmp({sub_events.TOA}, 'O') & strcmp({sub_events.block_type}, 'random'));
            elseif strcmp(conds{events_type_i},'Aintbk')
                curr_events = find(strcmp({sub_events.type}, 'BGIN'));
            else
                error('no such event type'); % or should it be just warning?
            end
...
for sub_i = 1:numel(subs)
    for sov_i=1:numel(sovs)
        ...
        for cond_i=1:numel(conds)
            ...
            if strcmp(conds{cond_i},"AblO")
                cond_name_for_trialsTable = 'AO';
            elseif  strcmp(conds{cond_i},"AblOF")
                cond_name_for_trialsTable = 'AOF';
            elseif  strcmp(conds{cond_i},"AblOR")
                cond_name_for_trialsTable = 'AOR';
            end
            trials_amount_table = load(sprintf("%s\\TrialsTable_cond-%s",event_statistics_dir,cond_name_for_trialsTable));
            trials_amount_table = trials_amount_table.trialsTable;
            sov_name = sovs{sov_i};
            if strcmp(sov_name,'wake_night')    sov_name = 'wn'; end
            subSovCond_number_of_trials = trials_amount_table{sov_name,subs{sub_i}};
...
folderPath = 'D:\OExpOut\processed_data\ft_processed'; % Specify the folder path
substring = 'noN2'; % Specify the substring to look for in file names
