...
set_output_dir = 'C:\Users\User\Cloud-Drive\BigFiles\OmissionExpOutput\import\interblock_baseline_extraction_my\';
...
events_type = {"Oprp","OFprp","ORprp",'interblockprp'};
...
for sub_i = 1:numel(subs)
    ...
    for files_i = 1:length(files_set)
        ...
        % resampling
        EEG_sub_file =  pop_resample( EEG_sub_file, new_sample_rate);
        ...
        for events_type_i=1:numel(events_type)
            ...
            for sov_i=1:numel(sovs)
                ...
                % remove bad channels
                EEG_currSovEvent = pop_select(EEG_sub_file,'nochannel',EEG_ica.reject.rejchan); 
                ...
                if strcmp(events_type{events_type_i},'interblock')
                    EEG_currSovEvent = pop_epoch(EEG_currSovEvent,{},[0 length_of_bl_epoch_sec],'eventindices',curr_sov_curr_eventtype_indexes);
                    EEG_currSovEvent = pop_rmbase(EEG_currSovEvent, []); % EEG_currSov = pop_rmbase(EEG_currSov, 'timerange',[0 200]);
                else
                    EEG_currSovEvent = pop_epoch(EEG_currSovEvent,{},[-0.1 0.45],'eventindices',curr_sov_curr_eventtype_indexes);
                ...
                % the data array
                if contains(sovs{sov_i}, 'wake') || strcmp(sovs{sov_i}, 'N1')
                    thresh_amp = 150;
                else
                    thresh_amp = 300;
                end
