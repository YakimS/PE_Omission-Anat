function trial_counts_table = analyze_trial_counts(subs, sovs, conds, ft_cond_input_dir, ft_cond_output_dir, time)
    ...
    %   trial_counts_table  - Cross-tabulated table with SOVs as rows, conditions as columns
        ...
        for sov_i = 1:numel(sovs)
            for cond_i = 1:numel(conds)
                ...
                trial_counts = zeros(numel(f.imp.subs), 1);
                
                % Get trial counts for each subject
                for sub_i = 1:numel(f.imp.subs)
                    try
                        % Get trials for current condition, subject, and SOV
                        curr_timelock = f.imp.get_cond_timelocked(f.imp, {f.imp.subs{sub_i}}, conds{cond_i}, sovs{sov_i});
