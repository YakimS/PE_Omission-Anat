function [rm_anova_results, parametric_posthoc, nonparametric_posthoc] = analyze_event_prevalence(subs, sovs, conds_sets, conds_sets_names, ft_cond_input_dir, ft_cond_output_dir, time, categories, plot_results)
...
%   nonparametric_posthoc - Table of non-parametric post-hoc test results (Wilcoxon)
    ...
    if nargin < 8 || isempty(categories) || ~iscell(categories) || length(categories) ~= 3
    ...
    for conds_sets_i = 1:numel(conds_sets)
        ...
        propor_data = zeros(numel(f.imp.subs), numel(conds), 3); % [subjects, conditions, categories]
        ...
        for cond_i = 1:numel(conds)
            for sub_i = 1:numel(f.imp.subs)
                ...
                curr_cond_timelockAll = f.imp.get_cond_timelocked(f.imp, {f.imp.subs{sub_i}}, conds{cond_i}, sovs{1});
                count_n2_all = curr_cond_timelockAll{1}.cfg.trials_timelocked_avg;
                
                % Process each category (KC, SS, Neither) in a loop
                for cat_i = 1:3
                    try
                        curr_timelock = f.imp.get_cond_timelocked(f.imp, {f.imp.subs{sub_i}}, conds{cond_i}, categories{cat_i});
