...
function batch_useAndrillonNirKcompDetection(input_dir,output_dir)
    ...
    for mat_file_ind=1:size(matFiles_input,2)
        ...
        output_filename = strcat(output_dir,'/KComp_',matFiles_input{mat_file_ind});
        ...
        % run thire code
        [kcomps] = KcompDetectionAlgorithm_fieldtrip(double(data),hdr, SleepScoring_vector);

        % save
        kcomps = kcomps{1};
        save(output_filename,"kcomps",'-double');
    end
end