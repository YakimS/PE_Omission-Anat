...
%%% https://github.com/andrillon/LSCPtools/blob/master/sleeptools/SpindlesDetectionAlgorithm_fieldtrip.m
%%%
%%% Input:
...
%%%         2500ms) durations of spindles in ms;
%%%         -- paramDetection.minFreqValue: minimum frequency value in Hz
%%%         used to compute spindle frequency (default: 11);
%%%         -- paramDetection.maxFreqValue: maximum frequency value in Hz
%%%         used to compute spindle frequency (default: 16);
...
if isfield(paramDetection,'spindleBand'),         spindleBand=paramDetection.spindleBand;                 else spindleBand=[11 16 4]; end % [11 16 4]
...
if isfield(paramDetection,'minMaxDurations_ms'),  minMaxDurations_ms=paramDetection.minMaxDurations_ms;   else minMaxDurations_ms=[500 2500]; end %[500 2500]
if isfield(paramDetection,'minFreqValue'),        minFreqValue=paramDetection.minFreqValue;               else minFreqValue=11; end % 11
if isfield(paramDetection,'maxFreqValue'),        maxFreqValue=paramDetection.maxFreqValue;               else maxFreqValue=16; end % 17
 ...
 continue;
  ...
  continue;
   ...
   continue;
        ...
        % Sum-up
        currentSpindle=[spindleStartTime spindleEndTime peakTime peakEnergy peakEnergyNorm freqSpindle spindleDuration/SR PowerSP PowerAlpha double(currentStage)];
