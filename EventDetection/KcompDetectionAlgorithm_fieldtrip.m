%%% SW detection algorithm of Andrillon, Nir at al.  Editted by Sharon
%%% Yakim to find k-complexes
%%%
% https://github.com/andrillon/LSCPtools/blob/master/sleeptools/SWsDetectionAlgorithm_fieldtrip.m

function [allWaves] = KcompDetectionAlgorithm_fieldtrip(data, hdr, SleepScoring_vector, paramDetection)
...
if isfield(paramDetection,'kCompBand'),              kCompBand=paramDetection.kCompBand;                           else kCompBand=[0.25 6 3]; end
...
%% Filter EEG
if length(kCompBand)==3
    datfilt=nan(length(ChannelSelection),size(data,2));
    for nch=ChannelSelection
        datfilt(nch,:)=bandpass(data(nch,:),SR,kCompBand(1),kCompBand(2),kCompBand(3));
    end
else
    datfilt=nan(length(ChannelSelection),size(data,2));
    for nch=ChannelSelection
        datfilt(nch,:)=lowpass(data(nch,:),SR,kCompBand(1),kCompBand(2));
    end
end
...
if SR~=SR_new
    for nch=ChannelSelection
        ...
        SleepScoring_vector=round(resample(double(SleepScoring_vector),SR_new,SR));
else
    datres=datfilt;
end
...
for nChan=ChannelSelection
    ...
    fprintf('... detecting kComplexes on %s (%g/%g)',hdr.label(nChan),countChan,length(ChannelSelection))
    ...
    EEGder=meanfilt(diff(EEGdata_BP_ss(1, :)),5); % replaces the value of each pixel by the average of all pixel values in a local neighborhood 
  ...
  if negcross(1)<poscross(1);start=1;else start=2;end %makes negcross and poscross same size to start
    ...
    for wndx=start:length(negcross)-1
        ...
        period=wavend-wavest; %matrix(11)
        ...
        waves(wndx, :) = [wavest wavend mdpt poszx period abs(b) bx c cx maxb2c n1 n1x nEnd nEndx p1 p1x meanAmp nump nperiod/SR p2p mxdn mxup thisStage];
        waves(wndx, [1 2 3 4 5 7 9 12 14 16])=waves(wndx, [1 2 3 4 5 7 9 12 14 16])/SR_new; % change sample to second
        
    end
    allWaves{nChan} = waves;

    fprintf('... %g potential kComps detected',size(allWaves{nChan},1))
    fprintf('\n')
end
end
