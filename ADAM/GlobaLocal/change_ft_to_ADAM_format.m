...
%%
ft_input_dir = 'D:\GlobalLocal\ft_subSovCond_blPreO';
...
for sub_ind=1:size(subs,2)
    ...
    merged_data.('elec') = subs_conds_ft{1}.elec; %otherwise, elec gets discarded, which causes problems
...
unexpomit_vals = 50 + (1:numel(subs));
expomit_vals = (1:numel(subs));
data_table_LOO = [unexpomit_vals', expomit_vals'];
disp('data_table:');
disp(data_table_LOO);
